...
app.get('/trails', async(req, res) => {
  try {
    const URL = `https://www.hikingproject.com/data/get-trails?lat=${req.query.lat}&lon=${req.query.longitude}&maxDistance=200&key=${process.env.TRAIL_KEY}`;

    const data = await request.get(URL);

    const newData = mungeTrail(data.body);
...
app.get('/reviews', async(req, res) => {
  try {
    const URL = `https://api.yelp.com/v3/businesses/search?lat=${req.query.latitude}&longitude=${req.query.longitude}`;

    const data = await request.get(URL)
      .set('Authorization', `Bearer ${process.env.REVIEWS_KEY}`);

    const newData = mungeYelp(data.body);
