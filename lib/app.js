...
//   try {
//     const URL = `https://api.yelp.com/v3/businesses/search?lat=${req.query.latitude}&lng=${req.query.longitude}`;
...
app.get('/reviews', async(req, res) => {
  try {
    const response = await request
      .get(
        `https://api.yelp.com/v3/businesses/search?lat=${req.query.latitude}&lng=${req.query.longitude}`
      )
      .auth(`${process.env.YELP_KEY}`, { type: 'bearer' });

    const munged = mungeReviews(response.body.businesses);
    res.json(munged);
  } catch(e) {
    res.status(500).json({
      status: 500,
...
app.get('/events', async(req, res) => {
  try {
    const eventful = await request
      .get(`http://api.eventful.com/json/events/search?app_key=${process.env.EVENTFUL_API_KEY}&where=${req.query.lat},${req.query.lng}&within=25`);
